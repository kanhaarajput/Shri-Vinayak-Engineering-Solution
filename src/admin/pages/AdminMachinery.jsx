      ...
      {isEditing && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-bold text-gray-900">{current ? 'Edit Machine' : 'Add New Machine'}</h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Specifications (Comma Separated)</label>
              <textarea
            ...
            <div>
              ...
              <div className="flex items-start gap-6">
                {preview && <img src={preview} alt="Preview" className="w-48 h-32 rounded-lg object-cover border border-gray-200" />}
                <input
            ...
            <div key={item.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="h-48 overflow-hidden relative">
                ...
                <div className="absolute top-2 right-2 flex gap-2">
                  <button onClick={() => handleEdit(item)} className="p-2 bg-white/90 text-gray-800 rounded-full hover:bg-amber-500 hover:text-white backdrop-blur-sm"><Pencil size={18} /></button>
                  <button onClick={() => { if (window.confirm('Delete this machine?')) deleteMachinery(item.id); }} className="p-2 bg-white/90 text-gray-800 rounded-full hover:bg-red-500 hover:text-white backdrop-blur-sm"><Trash2 size={18} /></button>
                </div>
              </div>
              <div className="p-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {item.specs && item.specs.map((spec, idx) => (
                    <span key={idx} className="bg-gray-100 text-gray-600 text-xs px-2 py-1 rounded-md font-medium">{spec}</span>
                  ))}
                </div>
              </div>
            </div>
